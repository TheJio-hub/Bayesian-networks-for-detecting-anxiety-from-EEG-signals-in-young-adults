...
def principal(tarea):
    dir_resultados = os.path.join('Resultados', 'Red Bayesiana', 'Tareas', tarea, 'Mixta')
    ...
    # Filtrar por Tarea Seleccionada y Relajación
        ...
        for algoritmo in ["PC", "GES"]:
            ...
            if dag_final_A is not None:
                try:
                    graficar_dag(
                        dag_final_A, 
                        f"Red Causal Mixta - {tarea} ({alg_desc}, Top-{n_caracteristicas})", 
                        os.path.join(dir_resultados, f"Estructura_{tarea}_{algoritmo}_top{n_caracteristicas}.png")
                    )
                except Exception as e:
                    print(f"Error graficando DAG {algoritmo} top {n_caracteristicas} para {tarea}: {e}")

    df_resultados = pd.DataFrame(resultados_loso).round(4)
    ruta_salida = os.path.join(dir_resultados, f'Resultados_Red_Mixta_{tarea}.csv')
    df_resultados.to_csv(ruta_salida, index=False)
    
    print(f"\n=== RESULTADOS RED CAUSAL MIXTA ({tarea.upper()}) ===")
    print(df_resultados.to_string(index=False, float_format=lambda x: f'{x:.4f}'))

if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Entrenamiento de Red Causal Mixta por Tarea Individual")
    parser.add_argument('--tarea', type=str, choices=['Aritmetica', 'Espejo', 'Stroop'],
                        help="Nombre de la tarea a evaluar contra Relajación (Aritmetica, Espejo, Stroop)")
    args = parser.parse_args()
    
    if args.tarea:
        principal(args.tarea)
    else:
        print("No se especificó una tarea con --tarea. Ejecutando todas las tareas secuencialmente...")
        for t in ['Aritmetica', 'Espejo', 'Stroop']:
            print("\n==========================================")
            print(f"Iniciando ejecución de la tarea: {t}")
            print("==========================================")
            principal(t)
