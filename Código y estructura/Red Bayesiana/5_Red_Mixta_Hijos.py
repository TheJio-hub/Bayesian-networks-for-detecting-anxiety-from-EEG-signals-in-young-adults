...
def obtener_hijos_y_copadres(dag, nodo_objetivo):
    if nodo_objetivo not in dag.nodes():
        return []
    hijos = set(dag.successors(nodo_objetivo))
    copadres = set()
    for hijo in hijos:
        padres_hijo = set(dag.predecessors(hijo))
        copadres.update(padres_hijo - {nodo_objetivo})
    return list(hijos.union(copadres))
...
def ejecutar_loso(df_datos, caracteristicas_seleccionadas, columna_objetivo, grupos, algoritmo):
    ...
    barra_progreso = tqdm(
        ...
        desc=f"LOSO Mixta Hijos {algoritmo}", 
        unit='fold', 
        leave=False
    )
    ...
    for indices_entrenamiento, indices_prueba in barra_progreso:
        ...
        try:
            ...
            variables_modelo = obtener_hijos_y_copadres(dag, columna_objetivo)
                
            if len(variables_modelo) == 0:
                raise ValueError("Conjunto de variables vacío")

            clf = LogisticRegression(class_weight='balanced', random_state=42)
            clf.fit(df_entrenamiento[variables_modelo], df_entrenamiento[columna_objetivo])
            
            predicciones = clf.predict(df_prueba[variables_modelo])
...
def principal():
    dir_resultados = os.path.join('Resultados', 'Red Bayesiana', 'Mixta_Hijos')
    ...
    for n_caracteristicas in tamanos_top:
        ...
        for algoritmo in ["PC", "GES"]:
            ...
            try:
                ...
                variables_modelo_final = obtener_hijos_y_copadres(dag_final, 'Ansiedad')
                    
                if len(variables_modelo_final) == 0:
                    raise ValueError("Conjunto de variables vacío")
                    
                clf_final = LogisticRegression(class_weight='balanced', random_state=42)
                clf_final.fit(df_desarrollo_sub[variables_modelo_final], df_desarrollo_sub['Ansiedad'])
                
                y_prueba_externa_pred = clf_final.predict(df_prueba_externa_sub[variables_modelo_final])
            except Exception:
            ...
            if dag_final is not None:
                try:
                    graficar_dag(
                        dag_final, 
                        f"Red Causal Mixta Hijos ({alg_desc}, Top-{n_caracteristicas})", 
                        os.path.join(dir_resultados, f"Estructura_Global_Hijos_{algoritmo}_top{n_caracteristicas}.png")
                    )
                except Exception as e:
                    print(f"Error graficando DAG {algoritmo} top {n_caracteristicas}: {e}")

    df_resultados = pd.DataFrame(resultados_loso)
    ruta_salida = os.path.join(dir_resultados, 'Resultados_Red_Mixta_Hijos.csv')
    df_resultados.to_csv(ruta_salida, index=False)
    
    print("\n=== RESULTADOS RED CAUSAL MIXTA HIJOS ===")
